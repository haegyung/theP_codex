...
2. ACP(예: `theprometheus-codex-acp`)를 실행하고 `/compact`, `/review`, `/undo`, `/init` 등 beta slash 명령을 순차적으로 실행합니다.
...
- `session_notification`을 디버깅하려면 `RUST_LOG=debug`를 활성화하고, `theprometheus-codex-acp`(또는 `cargo run --release`)을 실행해주세요.
